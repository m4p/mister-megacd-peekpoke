...
def main():
    ...
    try:
        ...
        check(j["core"].get("build_id") == "01260925" and j["core"].get("feature_bits") == 0x83, "build id / features from RTL PROBE")
        f = j["features"]
        check(f["bus_peek"] and f["input"] and not f["bus_poke"] and not f["pause"] and not f["state"]
        ...
        st, j = b.post("/pause")
        check(st == 501 and "freeze" in j["error"]["message"], "pause 501")
